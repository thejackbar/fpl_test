...
fantasy_team = fantasy_team.append(pd.DataFrame([{'Player Name': '', 'Position': 'GK', 'Price (M)': 0} for _ in range(2)]), ignore_index=True)
fantasy_team = fantasy_team.append(pd.DataFrame([{'Player Name': '', 'Position': 'DEF', 'Price (M)': 0} for _ in range(5)]), ignore_index=True)
fantasy_team = fantasy_team.append(pd.DataFrame([{'Player Name': '', 'Position': 'MID', 'Price (M)': 0} for _ in range(5)]), ignore_index=True)
fantasy_team = fantasy_team.append(pd.DataFrame([{'Player Name': '', 'Position': 'ATT', 'Price (M)': 0} for _ in range(3)]), ignore_index=True)
...
st.download_button(
    ...
    data=edited_team.to_excel(index=False),
